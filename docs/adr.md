...
**Context.** I had two finished Cityscapes segmentation projects, both in
TensorFlow/Keras, both ending at a trained model and a demo. The role I'm
targeting is about what happens after training: export, quantization,
benchmarking, embedded deployment, profiling. None of my work showed that.
...
repo is low, and the deployment loop is the core of the job.
...
**Context.** My prior models were Keras. The posting asks for PyTorch
explicitly. Cityscapes with the 8 top-level categories is the task I have
baselines for (0.786 mIoU, 60.5M-param U-Net).
...
full resolution - and the next architecture lever.
...
patterns that don't match silently no-op - assert your config applied.
...
output (no source-side handshake) - native DP it is, and a USB-TTL serial
...
cheaper per frame in energy - the opposite conclusion from the same model. It
...
## ADR-017: Repository hygiene as evidence

**Date:** 2026-08-17 · **Status:** accepted

**Context.** "Git, unit testing, debugging, profiling" is in the posting. A
repo is a better witness than a CV line.
...
deployment stack, which is the skill being assessed. This document exists
because that trail deserved a narrative.
...
- **Decoder width:** with QAT resolved, the profile's remaining lever is
...
- **C++ wrapper:** build on the Jetson, compare its latency with trtexec and
  the Python harness to size the Python overhead exactly.
